...
class ObjectCollisionEstimator : public rclcpp::Node
{
public:
  ObjectCollisionEstimator()
  : Node("ObjectCollisionEstimator")
  {
    subscription_ = this->create_subscription<reference_interfaces::msg::Message4kb>(
      "EuclideanClusterDetector", 1,
      std::bind(&ObjectCollisionEstimator::sub_callback, this, _1));
    publisher_ = this->create_publisher<reference_interfaces::msg::Message4kb>("ObjectCollisionEstimator", 1);
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<ObjectCollisionEstimator>());
  rclcpp::shutdown();
  return 0;
}
