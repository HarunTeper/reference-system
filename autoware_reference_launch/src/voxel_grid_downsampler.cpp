...
class VoxelGridDownsampler : public rclcpp::Node
{
public:
  ...
  : Node("VoxelGridDownsampler")
  {
    subscription_ = this->create_subscription<reference_interfaces::msg::Message4kb>(
      "PointCloudFusion", 1,
      std::bind(&VoxelGridDownsampler::sub_callback, this, _1));
    publisher_ = this->create_publisher<reference_interfaces::msg::Message4kb>("VoxelGridDownsampler", 1);
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<VoxelGridDownsampler>());
  rclcpp::shutdown();
  return 0;
}
