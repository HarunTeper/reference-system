...
class PointsTransformerFront : public rclcpp::Node
{
public:
  PointsTransformerFront()
  : Node("PointsTransformerFront")
  {
    subscription_ = this->create_subscription<reference_interfaces::msg::Message4kb>(
      "FrontLidarDriver", 1,
      std::bind(&PointsTransformerFront::sub_callback, this, _1));
    publisher_ = this->create_publisher<reference_interfaces::msg::Message4kb>("PointsTransformerFront", 1);
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<PointsTransformerFront>());
  rclcpp::shutdown();
  return 0;
}
