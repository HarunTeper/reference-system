...
class RayGroundFilter : public rclcpp::Node
{
public:
  RayGroundFilter()
  : Node("RayGroundFilter")
  {
    subscription_ = this->create_subscription<reference_interfaces::msg::Message4kb>(
      "PointCloudFusion", 1,
      std::bind(&RayGroundFilter::sub_callback, this, _1));
    publisher_ = this->create_publisher<reference_interfaces::msg::Message4kb>("RayGroundFilter", 1);
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<RayGroundFilter>());
  rclcpp::shutdown();
  return 0;
}
