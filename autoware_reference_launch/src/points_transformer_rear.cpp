...
class PointsTransformerRear : public rclcpp::Node
{
public:
  PointsTransformerRear()
  : Node("PointsTransformerRear")
  {
    subscription_ = this->create_subscription<reference_interfaces::msg::Message4kb>(
      "RearLidarDriver", 1,
      std::bind(&PointsTransformerRear::sub_callback, this, _1));
    publisher_ = this->create_publisher<reference_interfaces::msg::Message4kb>("PointsTransformerRear", 1);
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<PointsTransformerRear>());
  rclcpp::shutdown();
  return 0;
}
